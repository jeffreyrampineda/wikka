...
const SOKUON: { [char: string]: true } = { ゃ: true, ゅ: true, ょ: true };
 ...
 */
function checkSokuon(
  char: string,
  nextChar: string,
): { isSokuonTsu: boolean; isSokuonReg: boolean; input: string } {
  const result = {
    isSokuonTsu: char === SOKUON_TSU,
    isSokuonReg: Boolean(SOKUON[nextChar]),
